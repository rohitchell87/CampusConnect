...
export default function Profile() {
  ...
  if (!hasLoaded || !profile || !heroReady) {
    return (
      <div className="animate-fade-in">
        <div className="bg-[#08090F] w-full overflow-x-hidden">
          <div className="flex gap-3 w-full h-full">
          <div className="flex-1 space-y-3 px-3 py-4 min-w-0">
            <div className="h-36 rounded-[18px] bg-[#121424]/70 animate-pulse" />
            <div className="h-28 rounded-[18px] bg-[#121424]/70 animate-pulse" />
          ...
          <div className="w-56 space-y-3 flex-shrink-0 px-3 py-4">
            <div className="h-20 rounded-[18px] bg-[#121424]/70 animate-pulse" />
            <div className="h-20 rounded-[18px] bg-[#121424]/70 animate-pulse" />
            <div className="h-28 rounded-[18px] bg-[#121424]/70 animate-pulse" />
          </div>
        </div>
  ...
  return (
    <div className="animate-fade-in">
      <div className="min-h-screen bg-[#090B14] px-4 py-10 sm:px-6 lg:px-8">
        <div className="mx-auto w-[min(85vw,1500px)] max-w-[1500px] space-y-10">

        {/* HERO SECTION */}
        <section className="relative overflow-hidden rounded-[28px] bg-[#090B14] shadow-[0_35px_100px_rgba(0,0,0,0.45)]">
          <button
            type="button"
          ...
          >
            <div className="absolute inset-0">
              {coverPreview ? (
                ...
                <div className="h-full w-full bg-[#090B14]" />
              )}
            </div>
          </button>
          <div className="absolute inset-0">
            <div className="absolute inset-0 bg-gradient-to-t from-[#090B14]/95 via-[#090B14]/55 to-transparent" />
          </div>
              ...
              {!isOwnProfile && !previewOpen ? (
                <div className="relative">
                  <button
                    ...
                    className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-[#7C5CFF]/20 bg-[#0B0F1E]/90 text-white shadow-[0_12px_30px_rgba(124,92,255,0.15)] transition duration-200 hover:-translate-y-0.5 hover:bg-[#14182c]"
                  >
                    <MoreHorizontal className="h-5 w-5" />
                  </button>
                  {menuOpen && (
                    <div className="absolute right-0 mt-2 w-48 rounded-[18px] border border-white/10 bg-[#0B0F1E]/95 p-2 shadow-[0_20px_45px_rgba(0,0,0,0.3)]">
                      <button type="button" onClick={() => { handleShareProfile(); setMenuOpen(false) }} className="flex w-full items-center rounded-[12px] px-3 py-2 text-left text-sm text-[#EDE8FF] transition hover:bg-white/10">Share Profile</button>
                      <button type="button" onClick={() => { setReportOpen(true); setMenuOpen(false) }} className="flex w-full items-center rounded-[12px] px-3 py-2 text-left text-sm text-[#EDE8FF] transition hover:bg-white/10">Report User</button>
                      <button type="button" onClick={() => { setBlockOpen(true); setMenuOpen(false) }} className="flex w-full items-center rounded-[12px] px-3 py-2 text-left text-sm text-[#EDE8FF] transition hover:bg-white/10">Block User</button>
                    </div>
                  )}
                </div>
              ) : null}
              {!previewOpen && isOwnProfile ? (
                <>
                  <button
                    ...
                    className="inline-flex items-center justify-center rounded-full border border-[#7C5CFF]/20 bg-[#0B0F1E]/90 px-5 py-2 text-sm font-semibold text-white shadow-[0_12px_30px_rgba(124,92,255,0.15)] transition duration-200 hover:-translate-y-0.5 hover:bg-[#14182c]"
                  >
                    Share Profile
                  </button>
                  <button
                    ...
                    className="inline-flex items-center justify-center gap-2 rounded-full border border-[#7C5CFF]/20 bg-[#0B0F1E]/90 px-5 py-2 text-sm font-semibold text-white shadow-[0_12px_30px_rgba(124,92,255,0.15)] transition duration-200 hover:-translate-y-0.5 hover:bg-[#14182c]"
                  >
              ...
              <div className="flex items-end gap-6">
                <div className="relative">
                  {!previewOpen ? (
                    <button
                      ...
                      className="h-[120px] w-[120px] overflow-hidden rounded-full border-4 border-[#7C5CFF]/40 bg-[#090B14] shadow-[0_20px_40px_rgba(0,0,0,0.45)] cursor-zoom-in"
                    >
                    {profilePreview ? (
                    ...
                    <div className="h-[120px] w-[120px] overflow-hidden rounded-full border-4 border-[#7C5CFF]/40 bg-[#090B14] shadow-[0_20px_40px_rgba(0,0,0,0.45)]">
                      {profilePreview ? (
                        <img src={profilePreview} alt="Profile" className="h-full w-full object-cover" />
                      ) : profile?.profilePhoto ? (
                        <img src={profile.profilePhoto} alt="Profile" className="h-full w-full object-cover" />
                  ...
                  {!previewOpen ? (
                    <label className="absolute -bottom-2 -right-2 flex h-11 w-11 items-center justify-center rounded-full bg-gradient-to-r from-[#7C5CFF] to-[#A78BFA] text-white shadow-[0_12px_30px_rgba(124,92,255,0.35)] transition hover:brightness-110 cursor-pointer">
                ...
                <div className="min-w-0 text-white">
                  <div className="flex flex-wrap items-center gap-3 text-3xl font-bold tracking-tight">
                    <h1 className="truncate">{profile?.fullName || 'Your Name'}</h1>
                    <span className="text-xl font-semibold text-[#E6E6FA]">{profile?.age || '24'}</span>
                    {isVerified ? (
                      <VerifiedBadge user={profile?.id || 'self'} className="px-3 py-1" />
                    ) : (
                      <button type="button" onClick={() => setVerificationOpen(true)} className="rounded-full border border-[#2563EB]/25 bg-[#2563EB]/12 px-3 py-1 text-sm font-semibold text-[#DBEAFE] transition hover:bg-[#2563EB]/20">
                        Verify student
                      </button>
                    )}
                  </div>
                  <div className="mt-4 space-y-3 text-sm text-[#B9BBCC] sm:text-base">
                    ...
                    <div className="flex flex-wrap items-center gap-2 text-[#A9ABC1]">
                      <MapPin className="h-4 w-4 text-[#A78BFA] flex-shrink-0" />
          ...
          {coverPreview && (
            <div className="absolute right-6 top-6 z-10">
              <button type="button" onClick={uploadCover} className="rounded-full bg-gradient-to-r from-[#7C5CFF] to-[#A78BFA] px-4 py-2 text-xs font-semibold text-white shadow-[0_8px_24px_rgba(124,92,255,0.3)] transition hover:brightness-110">
        ...
        <section className="rounded-[28px] border border-white/10 bg-[#0B0F1F]/80 p-8 shadow-[0_24px_60px_rgba(0,0,0,0.35)] backdrop-blur-3xl">
          ...
          <div className="mt-6 text-[15px] leading-8 text-[#D8D4FF]">
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            {computedInterests.slice(0, 5).length > 0 ? (
              computedInterests.slice(0, 5).map((interest, index) => {
                ...
                return (
                  <span key={index} className="inline-flex items-center gap-2 rounded-full bg-[#111827] px-4 py-2 text-sm font-semibold text-white transition duration-200 hover:bg-[#1a1f38]">
              ...
              <div className="rounded-[24px] border border-dashed border-white/10 bg-[#101529] px-5 py-4 text-sm text-[#A9ABC1]">
        ...
        <section className="rounded-[28px] border border-white/10 bg-[#0B0F1F]/80 p-8 shadow-[0_24px_60px_rgba(0,0,0,0.35)] backdrop-blur-3xl">
          <div className="flex items-center justify-between gap-5">
            ...
            {!previewOpen && galleryPhotos.length > 0 ? (
              <button type="button" onClick={handleAddPhotosClick} className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-[#111827] px-4 py-2 text-sm font-semibold text-white transition duration-200 hover:bg-[#1c2140]">
        ...
        {/* DATING PROMPTS */}
        <section className="rounded-[28px] border border-white/10 bg-[#0B0F1F]/80 p-8 shadow-[0_24px_60px_rgba(0,0,0,0.35)] backdrop-blur-3xl">
          <h2 className="text-2xl font-semibold text-white">Dating Prompts</h2>
          <div className="mt-8 grid gap-4 md:grid-cols-3">
            {promptCards.length > 0 ? (
              promptCards.map((card, index) => (
                <div key={index} className="rounded-[24px] border border-white/10 bg-[#111827]/90 p-5 shadow-[0_20px_45px_rgba(0,0,0,0.22)] transition duration-200 hover:-translate-y-1">
                  <div className="flex items-center gap-2 text-sm font-semibold text-[#A78BFA]">
                    <Sparkles className="h-4 w-4" />
                    <span>{card.title}</span>
                  </div>
                  <p className="mt-4 text-sm leading-7 text-[#D8D4FF]">{card.text}</p>
                </div>
              ))
            ) : (
              <div className="grid gap-4 md:grid-cols-3">
                {['We’ll get along if...', 'Perfect Sunday...', 'Ideal vacation...'].map((title, index) => (
                  <div key={index} className="rounded-[24px] border border-white/10 bg-[#111827]/90 p-5 shadow-[0_20px_45px_rgba(0,0,0,0.22)] transition duration-200 hover:-translate-y-1">
                    ...
                    <p className="mt-4 text-sm leading-7 text-[#D8D4FF]">Add your own prompts to share what makes your vibe unique.</p>
                  </div>
                ))}
              </div>
            )}
        ...
        {/* PROFILE STATS */}
        <section className="rounded-[28px] border border-white/10 bg-[#0B0F1F]/80 p-8 shadow-[0_24px_60px_rgba(0,0,0,0.35)] backdrop-blur-3xl">
          <h2 className="text-2xl font-semibold text-white">Profile Stats</h2>
          <div className="mt-8 grid gap-4 sm:grid-cols-4">
            <div className="rounded-[24px] bg-[#111827]/90 p-6 text-center shadow-[0_20px_45px_rgba(0,0,0,0.22)]">
              <Heart className="mx-auto h-7 w-7 text-[#A78BFA]" />
              <div className="mt-4 flex items-center justify-center gap-3">
                <button
                  type="button"
                  onClick={() => setShowReasons((s) => !s)}
                  className="text-3xl font-bold text-white transition hover:text-[#EDE8FF]"
                >
                  {compatibilityScore}%
                </button>
              </div>
              <p className="mt-2 text-sm text-[#B9BBCC]">Compatibility</p>
            </div>
            <div className="rounded-[24px] bg-[#111827]/90 p-6 text-center shadow-[0_20px_45px_rgba(0,0,0,0.22)]">
              <Eye className="mx-auto h-7 w-7 text-[#A78BFA]" />
              <div className="mt-4 text-3xl font-bold text-white">{formatStat(profile?.views)}</div>
              <p className="mt-2 text-sm text-[#B9BBCC]">Profile Views</p>
            </div>
            <div className="rounded-[24px] bg-[#111827]/90 p-6 text-center shadow-[0_20px_45px_rgba(0,0,0,0.22)]">
              <ThumbsUp className="mx-auto h-7 w-7 text-[#A78BFA]" />
              <div className="mt-4 text-3xl font-bold text-white">{formatStat(profile?.likesReceived)}</div>
              <p className="mt-2 text-sm text-[#B9BBCC]">Likes</p>
            </div>
            <div className="rounded-[24px] bg-[#111827]/90 p-6 text-center shadow-[0_20px_45px_rgba(0,0,0,0.22)]">
              <Users className="mx-auto h-7 w-7 text-[#A78BFA]" />
              <div className="mt-4 text-3xl font-bold text-white">{formatStat(profile?.matches)}</div>
              <p className="mt-2 text-sm text-[#B9BBCC]">Matches</p>
            </div>
          </div>
        </section>
        <PhotoViewerModal
        ...
        <AnimatePresence>
          {previewOpen ? (
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="fixed inset-0 z-[80] overflow-y-auto bg-[#02040A]/85 px-3 py-4 backdrop-blur-xl sm:px-6 sm:py-6"
            >
              <motion.div
                ...
                className="mx-auto flex min-h-full w-full max-w-6xl flex-col rounded-[32px] border border-white/10 bg-[#080B14] shadow-[0_40px_140px_rgba(0,0,0,0.45)]"
              >
                <div className="flex items-center justify-between border-b border-white/10 px-4 py-4 sm:px-6">
                  <div>
                    <p className="text-sm font-semibold uppercase tracking-[0.3em] text-[#A78BFA]">Profile Preview</p>
                    <p className="mt-1 text-sm text-[#AAB0C2]">See your profile the way others will.</p>
                  </div>
                  <button type="button" onClick={() => setPreviewOpen(false)} className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-white/10 bg-white/5 text-white hover:bg-white/10">
                    <X className="h-5 w-5" />
                  </button>
                </div>
                  ...
                  {[
                    ...
                    return (
                      <button
                        ...
                        className={`inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-semibold transition ${active ? 'bg-gradient-to-r from-[#7C5CFF] to-[#A78BFA] text-white' : 'bg-white/5 text-[#D8D4FF] hover:bg-white/10'}`}
                      >
                ...
                <div className="flex-1 overflow-y-auto px-3 py-4 sm:px-6 sm:py-6">
                  {previewTab === 'profile-view' ? (
                    <div className="space-y-6">
                      <section className="overflow-hidden rounded-[28px] border border-white/10 bg-[#0B0F1F]/80 shadow-[0_24px_60px_rgba(0,0,0,0.35)]">
                        <div className="relative h-[220px] sm:h-[320px]">
                          ...
                          <div className="absolute inset-0 bg-gradient-to-t from-[#090B14]/95 via-[#090B14]/50 to-transparent" />
                          <div className="absolute left-6 right-6 bottom-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                            <div className="flex items-end gap-4">
                              <img src={profile?.profilePhoto || 'https://images.unsplash.com/photo-1544005313-94ddf0286df2?auto=format&fit=crop&w=600&q=80'} alt="Profile" className="h-24 w-24 rounded-full border-4 border-[#7C5CFF]/40 object-cover sm:h-28 sm:w-28" />
                              <div className="text-white">
                                <h2 className="text-2xl font-semibold">{profile?.fullName || 'Your Name'}</h2>
                                <p className="mt-1 text-sm text-[#D8D4FF]">{profile?.college || 'College'} • {profile?.branch || 'Branch'} • {profile?.year ? `Year ${profile.year}` : 'Year unknown'}</p>
                              </div>
                            </div>
                            <div className="rounded-full border border-[#7C5CFF]/20 bg-[#111827]/80 px-4 py-2 text-sm font-semibold text-[#EDE8FF]">{compatibilityScore}% compatible</div>
                          </div>
                        </div>
                        <div className="p-6 sm:p-8">
                          <div className="text-[15px] leading-8 text-[#D8D4FF]">{profile?.bio || 'No bio available yet.'}</div>
                          <div className="mt-6 flex flex-wrap gap-3">
                            {computedInterests.length > 0 ? computedInterests.slice(0, 5).map((interest, index) => {
                              const Icon = getInterestIcon(interest)
                              return <span key={index} className="inline-flex items-center gap-2 rounded-full bg-[#111827] px-4 py-2 text-sm font-semibold text-white"> <Icon className="h-4 w-4 text-[#A78BFA]" />{typeof interest === 'object' ? interest.name ?? interest.label ?? '' : interest}</span>
                            }) : <span className="rounded-full bg-[#111827] px-4 py-2 text-sm text-[#A9ABC1]">No interests listed</span>}
                          </div>
                        </div>
                      </section>
                      <section className="rounded-[28px] border border-white/10 bg-[#0B0F1F]/80 p-6 shadow-[0_24px_60px_rgba(0,0,0,0.35)] sm:p-8">
                        ...
                        <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                          {galleryPhotos.length > 0 ? galleryPhotos.slice(0, 6).map((photo, index) => (
                            ...
                            <div className="col-span-full rounded-[22px] border border-dashed border-white/10 bg-[#111827]/70 p-8 text-center text-sm text-[#A9ABC1]">No photos added yet.</div>
                          )}
                        </div>
                      </section>
                    </div>
                    ...
                    <div className="mx-auto flex max-w-[420px] justify-center">
                      <div className="w-full overflow-hidden rounded-[30px] border border-white/10 bg-[#0B0F1F]/85 shadow-[0_24px_80px_rgba(0,0,0,0.42)]">
                        <div className="relative h-[520px]">
                          ...
                          <div className="absolute left-5 right-5 top-5 flex items-center justify-between">
                            ...
                            <div className="rounded-full border border-[#7C5CFF]/30 bg-[#7C5CFF]/15 px-3 py-1 text-sm font-semibold text-[#EDE8FF]">{compatibilityScore}%</div>
                          </div>
                          <div className="absolute bottom-6 left-5 right-5">
                            <h3 className="text-3xl font-semibold text-white">{profile?.fullName || 'Your Name'}</h3>
                            <p className="mt-2 text-sm text-[#D8D4FF]">{profile?.college || 'College'} • {profile?.branch || 'Branch'} • {profile?.year ? `Year ${profile.year}` : 'Year unknown'}</p>
                            <div className="mt-4 flex flex-wrap gap-2">
                              {computedInterests.slice(0, 4).map((interest, index) => (
                                <span key={index} className="rounded-full border border-white/10 bg-white/10 px-3 py-1.5 text-sm text-[#F8FAFC]">{typeof interest === 'object' ? interest.name ?? interest.label ?? '' : interest}</span>
                              ))}
                            </div>
                          </div>
                        </div>
        ...
        {showReasons && !previewOpen && (
          <section className="rounded-[24px] border border-white/8 bg-[#0B0F1F]/70 p-6 shadow-[0_20px_45px_rgba(0,0,0,0.28)]">
